...
public class ConvertStringToObjectTest
{
    ...
    [Theory]
    [MemberData(nameof(TablesAndNameTableThatThrowArgumentException))]
    ...
    public static IEnumerable<object[]> TablesAndNameTableThatThrowArgumentException()
    {
        ...
        const string secondTableName = "Bank";
        var secondList = new List<DataTable>()
        {
            new()
        };
    ...
    public void GetDataTable_ShouldThrowArgumentException_WhenListOfTablesAreEmpty(List<DataTable> dataTables
        , string tableName)
    {
        // Arrange
        ...
        // Assert
        action.Should().Throw<ArgumentException>().WithMessage(
            ObjectCheck.EmptyListError(typeof(DataTable)));
    }
    
    public static IEnumerable<object[]> TablesThatThrowArgumentException()
    {
        const string firstTableName = "Student";
        var firstList = new List<DataTable>();
        yield return [firstList, firstTableName];
        
        var secondList = new List<DataTable>();
        yield return [secondList, null!];
    }
        ...
        , string aggregatedColumnName)
    {
        ...
        // Assert
        action.Should().Throw<ArgumentException>().WithMessage(
            ObjectCheck.NotExistColumnError(aggregatedColumnName));
    }
